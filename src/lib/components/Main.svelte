<script lang="ts">
	...
	let offsetX = 0;
	let offsetY = 0;
	let position = { top: 100, left: 100 }; // Initial position
	let minimize = false;

	let selectedWindow: 'settings' | 'hunt' | 'fight' | 'devTools' = 'settings';
...
<div
	style="
	  ...
	  width: {minimize ? '110' : '310'}px;
	  height: {minimize ? '32' : '200'}px;
...
>
	<div
		...
		user-select: none;
	  "
		on:pointerdown|stopPropagation={onPointerDownUI}
	>
		<button style="width: 73px; margin-right: 8px;" on:click={onClickMinimize}
			>{minimize ? 'Maximize' : 'Minimize'}</button
		>
		<label><input on:change={checkboxLoop} type="checkbox" />Loop</label>
		<button style="margin-left: 8px;" on:click={() => (selectedWindow = 'settings')}
			>Settings</button
		>
		<button on:click={() => (selectedWindow = 'hunt')}>Hunt</button>
		<button on:click={() => (selectedWindow = 'fight')}>Fight</button>
